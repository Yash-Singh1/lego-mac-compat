...
static int run_initializers(struct module32 *m)
{
    ...
    for (unsigned i = 0; i < m->segment_count; ++i) {
        ...
        for (uint32_t j = 0; j < s->nsects; ++j) {
            const struct section *sec = sections + j;
            if (!strncmp(sec->sectname, "__module_info", 16) && sec->size) {
                return fail("guest dylib Objective-C classes are not supported: %s", m->path);
            }
        }
    }
    for (unsigned i = 0; i < m->segment_count; ++i) {
...
int guest_dyld32_dispatch(const char *name, const uint32_t *a, uint64_t *result)
{
    /* COD4 loads PunkBuster at runtime, after loading Bink and its C++ runtime.
       Its dlopen/dlsym calls need the same i386 relocation and symbol handling;
       native dlopen cannot execute the original 32-bit modules. Other titles
       retain their native-library bridge until they initialize this loader. */
    if (!game_root[0]) return 0;
    if (!strcmp(name, "_dlopen")) {
        const char *path = (const void *)(uintptr_t)a[0];
        *result = guest_dyld32_open(path, (int)a[1]);
        if (getenv("LP32_TRACE_DYLD"))
            fprintf(stderr, "guest-dyld: dlopen %s -> 0x%08x\n", path ? path : "(main executable)", (uint32_t)*result);
    } else if (!strcmp(name, "_dlsym")) {
        *result = guest_dyld32_symbol(a[0], (const void *)(uintptr_t)a[1]);
    } else if (!strcmp(name, "_dlclose")) {
...
{
    /* Original test fixtures check relocation, binding, constructor ordering,
       paths, reuse and lookup without loading a game or opening a window. */
    pthread_mutex_lock(&loader_lock);
    error_pending = false;
    uint32_t launcher = open_locked("bin/fixture.dylib", false);
    uint32_t main = launcher ? guest_dyld32_symbol(launcher, "FixtureMain") : 0;
    unsigned count = module_count;
    bool ok = launcher && main && guest_dyld32_contains(main, 1) &&
              open_locked("bin/fixture.dylib", false) == launcher && module_count == count;
    if (ok) {
    ...
    if (ok && getenv("LP32_DYLD_FIXTURE_SELFTEST")) {
        for (unsigned attempt = 0; ok && attempt < 2; ++attempt) {
            ok = guest_dyld32_open("bin/fixture.dylib", RTLD_NOW) == launcher;
            if (ok) {
        ...
        if (ok) {
            ok = !guest_dyld32_open("bin/broken.dylib", RTLD_NOW) &&
                 ...
                 guest_dyld32_symbol(launcher, "FixtureMain") == main;
        }
    }
    fprintf(stderr, "guest-dyld self-test: %s (%u modules, FixtureMain=0x%08x)\n",
