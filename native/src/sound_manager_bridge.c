...
struct sound_channel {
    ...
    atomic_bool closed;
    bool playing, paused, owns_guest;
...
static int command(struct sound_channel *c,struct command32 cmd) {
    ...
    case 0: return 0;
    case 3: ++c->generation;if(c->audio)AudioQueueStop(c->audio,true);c->playing=false;return 0;
    case 4:
...
static void advance(struct sound_channel *c) {
    while(!c->closed && !c->playing && !c->paused && c->head) {
...
int sound_manager_bridge32_dispatch(const char *name,const uint32_t *a,uint64_t *out) {
    ...
    dispatch_block_t work=^{
        ...
        } else if(immediate) status=command(c,value);
        else {
