...
/* TFU 1.2's movie render loop calls NextFrame after DoFrame has already
   marked EOF. NextFrame then wraps FrameNum to zero, hiding completion from
   the non-looping movie manager at 0x734be2. Only guard this caller: loading
...
enum { TFU_MOVIE_END_HOOK = 0x734831, TFU_MOVIE_END_STUB = 0x7f00f000 };
static const uint8_t tfu_movie_end_expected[] = {
    0x89,0x1c,0x24,              /* mov [esp],ebx */
    0xe8,0xe9,0xae,0x96,0xff    /* call 0x9f722 */
};
...
static inline void tfu_movie_end_code(uint8_t code[28], uint8_t hook[8])
{
    const uint8_t body[28] = {
        ...
        0xe9,0,0,0,0,           /* continue: jmp 0x734839 */
        0x90
    };
    memcpy(code, body, sizeof(body));
    int32_t relative = (int32_t)(0x9f722u - (TFU_MOVIE_END_STUB + 22u));
    memcpy(code + 18, &relative, 4);
    relative = (int32_t)((TFU_MOVIE_END_HOOK + 8u) - (TFU_MOVIE_END_STUB + 27u));
    memcpy(code + 23, &relative, 4);
    memset(hook, 0x90, 8);
    hook[0] = 0xe9;
    relative = (int32_t)(TFU_MOVIE_END_STUB - (TFU_MOVIE_END_HOOK + 5u));
