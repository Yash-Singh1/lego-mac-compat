...
func publish(_ staged: URL, in directory: URL) throws -> URL {
    for suffix in 1...10000 {
        let name = suffix == 1 ? "TFU-Compat.app" : "TFU-Compat \(suffix).app"
...
final class Converter {
    ...
    static var defaultCache: URL {
        FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("org.32bitgoofy.TFUConverter")
    }
    ...
    func run(_ executable: String, _ arguments: [String], cwd: URL? = nil,
        ...
        let log = fm.temporaryDirectory.appendingPathComponent("tfu-command-\(UUID().uuidString).log")
    ...
    func convert(source: URL, destination: URL) throws -> URL {
        ...
        let layout = try TFUSource.discover(source)
        try layout.checkDestination(destination)
        let image = layout.image
        progress("Checking this Mac build…", "Verifying the compatibility routines before copying your game.")
        try run("/usr/bin/arch", ["-x86_64", resources.appendingPathComponent("game_loader").path,
                                  "--validate-tfu", image.path])
        try cancellation.check()
        ...
        let temp = destination.appendingPathComponent(".tfu-converting-\(UUID().uuidString)")
        try fm.createDirectory(at: temp, withIntermediateDirectories: false)
        defer { try? fm.removeItem(at: temp) }
        let app = temp.appendingPathComponent("TFU-Compat.app")
        ...
        progress("Copying The Force Unleashed…", "Copying your game into a separate app. Your existing TFU saves stay in their original location.")
        try layout.copyGame(to: output.appendingPathComponent("SharedSupport/TFU"), check: cancellation.check)
        try run("/usr/bin/ditto", ["--noextattr", "--noqtn", layout.resources.path,
                                  output.appendingPathComponent("Resources").path])
        try cancellation.check()
        try fm.copyItem(at: image, to: output.appendingPathComponent("SharedSupport/TFU.image"))
        try fm.setAttributes([.posixPermissions: 0o644], ofItemAtPath: output.appendingPathComponent("SharedSupport/TFU.image").path)
        let runtimeOutput = output.appendingPathComponent("SharedSupport/TFU/compat-runtime")
        ...
        try fm.copyItem(at: resources.appendingPathComponent("game_loader"), to: output.appendingPathComponent("MacOS/TFUCompat"))
