...
uint32_t compat_runtime32_call(uint32_t fn,const uint32_t *a,size_t n){(void)fn;(void)a;(void)n;return 0;}
...
int main(void){
    ...
    puts("Steam bridge PASS (register/sret SteamID, old/new storage slots, quota canaries, screenshot callback hook)");
}
